	...
	if (!(@file_exists($ss."/list.txt"))){
    	...
    	foreach($dirs as $fil){
          		...
          		if($cnt<$page*16||$cnt>=($page+1)*16){
                  	echo "<a href=\"".$ss."/" .str_replace("__highres","",$fil)."\" data-lightbox='example-set' title=\"(<a href='down.php?file_dir=".urlencode($ss)."&file_name=" .urlencode($fil)."'>下載</a>) <a href='".$ss."/" .$fil."'>" .$fil."</a>\"/>"; 
                 	}
              	if($cnt==$page*16){
      				echo"<table border='0'>";
              		}
              	if($cnt>=$page*16&&$cnt<($page+1)*16){
                  	...
                  	if($cnt%4==0)echo "<tr>";
                  	echo "<td width='130'><a class='example-image-link' href=\"".$ss."/" .str_replace("__highres","",$fil)."\" data-lightbox='example-set' title=\"(<a href='down.php?file_dir=".urlencode($ss)."&file_name=" .urlencode($fil)."'>下載</a>) <a href='".$ss."/" .$fil."'>" .$fil."</a>\"><img class='example-image' height=\"90\" weight=\"160\" src=\"".$ss."/" .str_replace("__highres","",$fil)."\"></a>"; 
                	}
              	$cnt++;
            	}
		...
		for($a=0;$a<($page*16);$a++){
			...
			if($Talk==""||$Talk=="\n"){break;}
			$Ntk=trim($Talk);
          echo "<a href=\"".$ss."/" .str_replace("__highres","",$Ntk)."\" data-lightbox='example-set' title=\"(<a href='down.php?file_dir=".urlencode($ss)."&file_name=" .urlencode($Ntk)."'>下載</a>) <a href='".$ss."/" .$Ntk."'>" .$Ntk."</a>\"/>"; 
			}
      	echo"<table border='0'>";
		for($a=0;$a<16;$a++){
			$Talk = fgets($fileopen,1000);
          ...
          //href=\"smallpic.php?src=".$ss."/" .$Ntk."&h=768&zc=1\"
			echo "<td width='130'><a class='example-image-link' href=\"".$ss."/" .str_replace("__highres","",$Ntk)."\" data-lightbox='example-set' title=\"(<a href='down.php?file_dir=".urlencode($ss)."&file_name=" .urlencode($Ntk)."'>下載</a>) <a href='".$ss."/" .$Ntk."'>" .$Ntk."</a>\"><img class='example-image' height=\"90\" weight=\"160\" src=\"".$ss."/" .str_replace("__highres","",$Ntk)."\"></a>"; 
			$f++;
			}
      	$t=0;
      	echo"</td></tr><tr><td>";
